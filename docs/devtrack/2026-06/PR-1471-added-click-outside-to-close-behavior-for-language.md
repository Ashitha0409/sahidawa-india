...
1.  **`mousedown` vs. `click` for Outside Detection:** We opted to use the `mousedown` event listener instead of `click` for detecting outside interactions. `mousedown` fires earlier in the event propagation cycle than `click`. This choice helps prevent potential race conditions or unintended side effects where a `click` event on an element outside the dropdown might trigger its own action _before_ the dropdown has a chance to close, leading to a less predictable user experience.
...
1.  **Click Outside:**
    ...
    - Click anywhere on the page _outside_ the dropdown menu.
...
3.  **Click Inside Dropdown:**
    - Open the language switcher dropdown.
    - Click on any of the language options _inside_ the dropdown.
