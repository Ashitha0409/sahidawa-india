...
- Ensure `package.json` and `package-lock.json` (and the service-specific `package.json`) are copied _before_ `npm ci` in the `base` stage to maximize Docker layer caching.
