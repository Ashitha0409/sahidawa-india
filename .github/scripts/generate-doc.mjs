...
if (!GEMINI_API_KEY) {
    console.error(
        "❌ GEMINI_API_KEY is not set. Add it as a GitHub Actions secret named GEMINI_API_KEY."
    );
    process.exit(1);
}
...
async function callGemini(prompt) {
    ...
    if (candidate.finishReason && candidate.finishReason !== "STOP") {
        console.warn(
            `⚠️ Warning: Gemini generation stopped early! Reason: ${candidate.finishReason}`
        );
    }
    ...
    if (!text.trim()) {
        console.error(
            "Gemini API returned empty text. Full response:",
