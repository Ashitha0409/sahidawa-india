...
function score() {
    ...
    if (prLabels.includes("skip-devtrack")) {
        return {
            score: 0,
            verdict: "SKIP",
            reason: "Label 'skip-devtrack' applied by maintainer",
            area: "N/A",
        };
    }
    ...
    if (
        ...
        return {
            score: 0,
            verdict: "SKIP",
            reason: "Dependabot dependency update — no architectural docs needed",
            area: "N/A",
        };
    }
    ...
    if (linesChanged < 20) {
        return {
            score: 0,
            verdict: "SKIP",
            reason: `Only ${linesChanged} lines changed (threshold: 20)`,
            area: "N/A",
        };
    }
    ...
    if (allFilesAreDocsOnly(filesChanged)) {
        return {
            score: 0,
            verdict: "SKIP",
            reason: "Only documentation files changed (README, docs/, templates)",
            area: "N/A",
        };
    }

    if (hasSkipCommitType(commitMsgs)) {
        const matched = commitMsgs.find((m) =>
            SKIP_COMMIT_TYPES.some((p) => m.trim().toLowerCase().startsWith(p))
        );
        return {
            score: 0,
            verdict: "SKIP",
            reason: `Commit type skipped: "${matched}"`,
            area: "N/A",
        };
    }
    ...
    const isOnlyFixes =
        commitMsgs.length > 0 && commitMsgs.every((m) => m.trim().toLowerCase().startsWith("fix:"));
    ...
    if (totalScore < 5) {
        return {
            score: totalScore,
            verdict: "SKIP",
            reason: `Impact score ${totalScore} below threshold (5)`,
            area,
        };
    }
    ...
    if (totalScore >= 15) {
        return {
            score: totalScore,
            verdict: "GENERATE+ADR",
            reason: `High architectural impact (score: ${totalScore})`,
            area,
        };
    }
    ...
    return {
        score: totalScore,
        verdict: "GENERATE",
        reason: `Meaningful change (score: ${totalScore})`,
        area,
    };
}
