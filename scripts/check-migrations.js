...
function parseMigrations(migrationsDir) {
    ...
    if (!fs.existsSync(migrationsDir)) {
        console.error(`${c.red}✖${c.reset} Migrations directory not found: ${migrationsDir}`);
        console.error(`  Create the directory and add your .sql migration files there.`);
        process.exit(1);
    }
...
function parseAlterTableAddColumn(sql, schema) {
    ...
    while ((tableMatch = alterTableRegex.exec(sql)) !== null) {
        ...
        const addColRegex = /ADD\s+(?:COLUMN\s+)?(?:IF\s+NOT\s+EXISTS\s+)?(\w+)\s+\w/gi;
        let colMatch;
...
function parseCreatePolicy(sql, schema) {
    ...
    const policyRegex = /CREATE\s+POLICY\s+["']?(\w+)["']?\s+ON\s+(?:\w+\.)?(\w+)/gi;
    let match;
...
function main() {
    ...
    if (!fs.existsSync(TARGET_SCHEMA_PATH)) {
        console.error(`${c.red}✖${c.reset} Target schema not found: ${TARGET_SCHEMA_PATH}`);
        console.error(
